...
def compra_por_id(sesion:Session,id_compra:int):
    ...
    return sesion.query(modelos.Usuario).filter(modelos.Usuario.id==id_compra).first()

def foto_por_id(sesion:Session,id_foto:int):
    print("select * from app.fotos where id = ", id_foto)
    return sesion.query(modelos.Usuario).filter(modelos.Usuario.id==id_foto).first()
